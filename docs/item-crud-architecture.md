...
A unified CRUD design that serves all 7 system item types (snippet, prompt, command, note, link, file, image) through **one set of mutations**, **one query module**, **one dynamic route**, and **shared components that adapt by type**. Type-specific logic lives in components — never in the actions or query layer.

See also: [docs/item-types.md](./item-types.md) for the type catalog and the `text` / `url` / `file` content-type classification referenced throughout.

---

## Principles

1. **One Item, many faces.** All 7 types are rows in `Item`, discriminated by `itemTypeId` and `contentType`. There is no per-type table, so there should be no per-type action file.
...
└── components/
    └── items/
        ...
        │   └── FileUploadField.tsx   # R2 upload (file + image)
        └── viewers/
            ├── TextViewer.tsx        # syntax-highlighted code / markdown
            ├── UrlViewer.tsx         # clickable preview card
            ├── FileViewer.tsx        # filename + size + download
            └── ImageViewer.tsx       # inline thumbnail + download
```

**Why one `actions/items.ts`:** every type mutates the same `Item` table. A per-type action file would be 7× duplication of identical Prisma calls.
...
export function generateStaticParams() {
  return [
    ...
    { type: "notes" },    { type: "links" },   { type: "files" },
    { type: "images" },
  ];
}
```

`resolveTypeSlug` lives in `lib/items/type-registry.ts` and maps plural URL slugs to the canonical singular name stored in `ItemType.name` from [prisma/seed.ts](../prisma/seed.ts). It also exposes the `contentType` (`text` / `url` / `file`) so the page and its components know which field set to render.
...
- Validates with a **Zod discriminated union** on `contentType` — the schema rejects mismatched fields (e.g. `content` submitted for a `file` type).
...
export const CONTENT_VIEWERS = {
  text: TextViewer,
  url:  UrlViewer,
  file: FileViewer,     // image also resolves to file, but the image type overrides below
} as const;

export const TYPE_VIEWERS = {
  image: ImageViewer,   // overrides the file default
} as const;
...
Resolution order in the drawer / form:
1. Look up by specific type name (`image` → `ImageViewer`).
2. Fall back to `contentType` (`text` / `url` / `file`).

This keeps the 3-way `text | url | file` split as the primary switch and reserves per-type overrides for genuine differences (image thumbnails vs generic file download).
...
| `FileUploadField` | Drag-and-drop upload to R2, surfaces progress. Writes `fileUrl`/`fileName`/`fileSize`. | N/A. |
| `TextViewer`      | Syntax highlight (by `language`) or markdown render.                           | N/A. |
| `UrlViewer`       | Clickable preview card.                                                        | N/A. |
| `FileViewer`      | Filename, size, download button.                                               | N/A. |
| `ImageViewer`     | Inline thumbnail + download (extends `FileViewer`).                            | N/A. |

Anything that needs color, icon, or Pro-gating reads it from the `ItemType` join — never from a hardcoded switch on the type name.
...
Swap `contentType: "file"` and the same machinery handles an image upload — only the field component and the Zod branch differ. No new actions, no new routes, no new queries.

---

## Extending to custom types (future)

Custom types become `ItemType` rows with `isSystem = false` and a `userId`. They still resolve to one of the three `contentType` values, so:
